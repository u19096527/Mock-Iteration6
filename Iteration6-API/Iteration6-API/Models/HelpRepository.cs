...
using static Iteration6_API.Models.HelpRepository;
...
namespace Iteration6_API.Models
{
    public class HelpRepository : IHelpRepository
    {
            ...
            {
                IQueryable<Help> query = _appDbContext.HelpTips;
                return await query.ToArrayAsync();
            }

            public async Task<Help> GetAHelpTipAsync(int Help_ID)
            {
                IQueryable<Help> query = _appDbContext.HelpTips.Where(c => c.Help_ID == Help_ID);
                return await query.FirstOrDefaultAsync();
            }

            public async Task<Help[]> GetSearchedHelpTipAsync(string enteredQuery)
            {
                IQueryable<Help> query = _appDbContext.HelpTips.Where(c => c.Name.Contains(enteredQuery)
